...
router.post("/login", async (req, res) => {
  ...
  try {
      ...
      res.status(200).json({
          ...
          memberships: role === "user" ? user.memberships : undefined,
...
router.get("/get-user", authenticateToken, async (req, res) => {
  try {
    let user_id = req.query.id || req.user?.user_id; // Get user ID from query param or token

    if (!user_id) {
      return res.status(400).json({ message: "User ID or token is required." });
    }

    const user = await Userlogin.findById(user_id); // Fetch user by ID
...
router.get("/get-all-users-data",  async (req, res) => {
  try {
    ...
    const user = await Userlogin.find({}); // Use user ID to find the user
